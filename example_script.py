...
start = time()
serial_python_result = simple_pairwise_sum_python(x, y)
end = time()
print("\n\nTotal runtime for serial pairwise_sum_python = {0:.1f} ms".format((end-start)*1000.))

start = time()
serial_cython_result = simple_pairwise_sum_cython(x, y)
end = time()
print("Total runtime for serial simple_pairwise_sum_cython = {0:.1f} ms\n\n".format((end-start)*1000.))
